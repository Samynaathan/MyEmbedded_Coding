...
from python_log_lib import PyCanoeLogger
...
class CANoe:
    ...
    def __init__(self, py_canoe_log_dir='', user_capl_functions=tuple()):
        try:
            ...
            self.measurement_start_stop_timeout = 60  # default timeout value
            ...
            # Initialize Measurement COM Object
            self.measurement_com_obj = self.application_com_obj.Measurement
            self.__log.debug("CANoe Measurement COM object initialized successfully.")
    ...
    def __init_canoe_application(self):
        try:
            ...
            # Set up application open/close event handlers
            self.wait_for_canoe_app_to_open = lambda: DoMeasurementEventsUntil(
                lambda: CANoe.CANOE_APPLICATION_OPENED, lambda: self.application_open_close_timeout
            )
            self.wait_for_canoe_app_to_close = lambda: DoMeasurementEventsUntil(
                lambda: CANoe.CANOE_APPLICATION_CLOSED, lambda: self.application_open_close_timeout
            )

            # Enable application events if configured
            if self.application_events_enabled:
                win32com.client.WithEvents(self.application_com_obj, CanoeApplicationEvents)
            wait(0.5)
            self.__log.debug("CANoe application initialized successfully.")
        except Exception as e:
